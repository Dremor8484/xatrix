...
Pickup_Bandolier(edict_t *ent, edict_t *other)
{
	...
	if (sv_custom_settings->value)
	{
		...
		if (other->client->pers.max_magslug < cs_bandolier_ammo_magslug->value) //75
		{
			other->client->pers.max_magslug = cs_bandolier_ammo_magslug->value; //75
		}
	}
...
Pickup_Pack(edict_t *ent, edict_t *other)
{
	...
	if (sv_custom_settings->value)
	{
		...
		if (other->client->pers.max_slugs < cs_ammopack_ammo_magslug->value) //100
		{
			other->client->pers.max_slugs = cs_ammopack_ammo_magslug->value; //100
		}
		
		if (other->client->pers.max_trap < cs_ammopack_ammo_trap->value) //5
		{
			other->client->pers.max_trap = cs_ammopack_ammo_trap->value; //5
		}			
	}
...
Use_QuadFire(edict_t *ent, gitem_t *item)
{
	...
	int quadfire_duration = 300;
	if (sv_custom_settings->value)
	{
		quadfire_duration = cs_quadfire_duration->value; //300
	}
...
SP_item_foodcube(edict_t *self)
{
	...
	if (sv_custom_settings->value && cs_health_food_val->value)
	{
		self->count = cs_health_food_val->value; //?			
	}
